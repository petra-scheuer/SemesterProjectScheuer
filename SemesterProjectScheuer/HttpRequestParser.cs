...
public class HttpRequestParser
{
    public static async Task<HttpRequest> ParseFromStreamAsync(Stream stream)
    {
        ...
        while (!string.IsNullOrEmpty(line = await reader.ReadLineAsync()))
        {
            ...
            {
                 var contentLegth = line.Substring("Content-Length:".Length).Trim();
                 int.TryParse(contentLegth, out contentLength);  // anm. out contentLength -> gibt an wie lange der Body ist, nötig um zu wissen wie viele Bytes nach dem HTTP-Header noch eingelesen werden müssen
            }
        }
